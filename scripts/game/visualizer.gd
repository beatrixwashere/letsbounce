...
func _draw() -> void:
	...
	for i in VU_COUNT:
		...
		draw_rect(
				Rect2(w * i, HEIGHT - height, w - 4, height),
				#Color.from_hsv(float(VU_COUNT * 0.6 + i * 0.5) / VU_COUNT, 0.5, 0.6)
				Color.from_hsv(float(VU_COUNT * 0.6 + i * 0.5) / VU_COUNT, float(VU_COUNT * 0.6 + i * 0.5) / VU_COUNT, 0.3)
		)
		draw_line(
		...
		draw_rect(
				...
				#Color.from_hsv(float(VU_COUNT * 0.6 + i * 0.5) / VU_COUNT, 0.5, 0.6)
				Color.from_hsv(float(VU_COUNT * 0.6 + i * 0.5) / VU_COUNT, float(VU_COUNT * 0.6 + i * 0.5) / VU_COUNT, 0.2)
		)
		draw_line(
				Vector2(w * i, HEIGHT + height),
				Vector2(w * i + w - 2, HEIGHT + height),
